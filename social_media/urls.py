...
urlpatterns = [
    ...
    url(r'^connected/(?P<user_id>[\w\-]+)$', views.ConnectedNgoPeople.as_view(), name='connected_ngo_people'),
    url(r'^messages$', views.SocialMediaMessage.as_view(), name='social_media_messages'),
    url(r'^timeline/(?P<user_id>[\w\-]+)$', views.UserTimeline.as_view(), name='user_timeline'),
    url(r'^about/(?P<user_id>[\w\-]+)$', views.TimelineAbout.as_view(), name='user_timeline_about'),
